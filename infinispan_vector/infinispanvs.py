...
class Infinispan:
    ...
    def req_query_post(self, query_str, cache_name, local=False) -> requests.Response:
        api_url = self._default_node + self._cache_url + "/" + cache_name + "?action=search"
    ...
    def req_query_get(self, query_str, local=False) -> requests.Response:
        api_url = self._default_node + self._cache_url + "/" + self._cache_name + "?action=search&query=" + query_str + "&local=" + str(
            local)
...
class InfinispanVS(VectorStore):
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]] = None, **kwargs: Any) -> List[str]:
        ...
        for text, metadata, embed in data_input:
            ...
            resp = self.ispn.req_put(key, dataStr, self._cache_name)
            result.append(key)
        return result
    ...
    def from_texts(
            ...
            ispn: Optional[Infinispan] = None,
            **kwargs: Any
    ) -> InfinispanVS:
        """Return VectorStore initialized from texts and embeddings."""
        infinispan = cls(embedding=embedding, configuration=configuration, ispn=ispn)
        if texts:
