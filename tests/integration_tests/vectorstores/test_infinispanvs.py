...
class TestBasic:
    def test_infinispan(self, autoconfig) -> None:
    ...
    def test_infinispan_with_metadata(self, autoconfig) -> None:
        """Test with metadata"""
        if not autoconfig:
            _infinispan_setup_noautoconf()
        meta = []
    ...
    def test_infinispan_with_metadata_with_output_fields(self, autoconfig) -> None:
        """Test with metadata"""
        if not autoconfig:
            _infinispan_setup_noautoconf()
        metadatas = [{"page": i, "label": "label" + str(i)} for i in range(len(fake_texts))]
        c = {"output_fields": ["label", "page", "text"]}
        docsearch = _infinispanvs_from_texts(metadatas=metadatas, configuration=c, auto_config=autoconfig)
        output = docsearch.similarity_search("foo", k=1)
        assert output == [
            Document(page_content="foo", metadata={"label": "label0", "page": 0})
        ]

    def test_infinispanvs_with_id(self, autoconfig) -> None:
        """Test with ids"""
    ...
    def test_infinispan_with_score(self, autoconfig) -> None:
        """Test end to end construction and search with scores and IDs."""
        if not autoconfig:
            _infinispan_setup_noautoconf()
        texts = ["foo", "bar", "baz"]
        metadatas = [{"page": i} for i in range(len(texts))]
        docsearch = _infinispanvs_from_texts(metadatas=metadatas, auto_config=autoconfig)
    ...
    def test_infinispan_add_texts(self, autoconfig) -> None:
    ...
    def test_infinispan_no_clear_old(self, autoconfig) -> None:
        """Test end to end construction and MRR search."""
        if not autoconfig:
            _infinispan_setup_noautoconf()
        texts = ["foo", "bar", "baz"]
        ...
        del docsearch
        try:
            docsearch = _infinispanvs_from_texts(metadatas=metadatas, clear_old=False, auto_config=autoconfig)
